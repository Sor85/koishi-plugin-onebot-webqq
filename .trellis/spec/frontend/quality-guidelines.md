...
- 窄屏媒体查询只能调整固定定位的 `.onebot-webqq-host` 和 `.onebot-webqq__body`，不要给相对定位的 `.onebot-webqq` 写 `right` 或 `bottom`；否则视觉胶囊会相对 layout root 偏移，但正文仍按 fixed 锚点停在原位，导致文字脱离胶囊。
- 多机器人头像组的展开、折叠和切换位置动画必须使用 Anime.js layout 的 `record()` / `animate()`，不要用 CSS `right`、`width`、`transform` transition 手写布局动画；固定定位只能放在 `.onebot-webqq-host` 上，layout root 必须是 `.onebot-webqq-layout-root`，它的 DOM 子树只能包含外层视觉胶囊 `.onebot-webqq`、左侧 `.onebot-webqq__avatar-capsule` 和头像区域，不能包含右侧正文。这样 Anime.js 能记录外层右锚定胶囊向左扩张的位移，用来抵消当前头像在头像组内的右移，保证当前头像屏幕坐标不动，同时避免 fixed/absolute root 触发 Anime.js 的绝对坐标路径导致头像层脱离原位。`.onebot-webqq-host` 必须固定为 `height: 52px` 且 `line-height: 0`，`.onebot-webqq-layout-root` 必须是 `display: block` 且固定同高；否则 Anime.js 动画中临时把视觉胶囊改成 absolute 时，inline baseline 行盒会改变 fixed host 高度，导致 bottom 锚定的整颗胶囊上下跳。`children` 必须记录外层胶囊、头像胶囊、头像栈、单个头像按钮和余量数字；其中 `.onebot-webqq__bot-switch` 必须参与 FLIP，才能抵消外层胶囊向左扩张时对子节点的位移，保证当前 bot 头像屏幕坐标不动。`.onebot-webqq__avatar-guide` 只属于单机器人路径，不能放进 layout children，否则多机器人折叠态可能把波纹层带进头像组动画。
- 余量数字头像必须和头像保持相同大小、宽度与折叠步进，放在最后一个可见头像的下层；由于右侧会被下一张头像遮住，`+N` 文本必须拆成 `+` 和数字两个片段，并在左侧露出的 24px 区域内水平/垂直居中，确保加号和数字对齐且可见，不能浮在头像上层。
- 多机器人头像组无论折叠还是展开，都只在当前选中的机器人头像上显示在线状态点。
- 头像图形引导仅用于单机器人头像；多机器人头像组内不能渲染 `.onebot-webqq__avatar-guide`，否则折叠态最右侧当前 bot 会出现波纹扩散效果。多机器人时当前机器人头像固定在最右侧。
