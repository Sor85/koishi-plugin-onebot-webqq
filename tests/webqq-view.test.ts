...
const webqqMessageReactionsView = await readFile(new URL('../client/components/WebQQMessageReactions.vue', import.meta.url), 'utf8')
...
describe('webqq observer view', () => {
  ...
  it('renders contacts, groups, message history, and no send input', () => {
    ...
    expect(webqqView).not.toContain("send('onebot-webqq/webqq/send'")
  ...
  it('renders completed thinking below WebQQ messages as a collapsible disclosure', () => {
    ...
    expect(webqqMessageListView).toContain('<Transition name="onebot-webqq-webqq-thinking" @after-leave="animatePendingThinkingLeaveMove">')
    ...
    expect(webqqMessageListView).toContain('function animatePendingThinkingLeaveMove()')
    expect(webqqMessageListView).toContain('const previousRects = readMessageRowRects()')
    expect(webqqMessageListView).toContain('pendingThinkingLeaveRects = previousRects')
