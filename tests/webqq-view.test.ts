...
const webqqFrostedSurface = await readFile(new URL('../client/webqq/utils/webqq-frosted-surface.ts', import.meta.url), 'utf8')
...
const webqqMessageSearchPage = await readFile(new URL('../client/webqq/components/WebQQMessageSearchPage.vue', import.meta.url), 'utf8')
...
describe('webqq observer view', () => {
  ...
  it('uses the configured WebQQ frosted glass option without rendering an in-panel theme selector', () => {
    ...
    expect(webqqView).toContain("from './utils/webqq-frosted-surface'")
    expect(webqqView).toContain('useWebQQFrostedSurfaceFlag()')
    expect(webqqFrostedSurface).toContain("document.body.dataset.onebotWebqqFrosted = 'true'")
    expect(webqqFrostedSurface).toContain('delete document.body.dataset.onebotWebqqFrosted')
    expect(webqqFrostedSurface).not.toContain('dataset.sandboxFrosted')
    ...
    expect(webqqView).toContain('`is-color-${resolvedWebQQColorMode}`')
  ...
  it('uses the configured WebQQ color mode without rendering an in-panel switcher', () => {
    ...
    expect(webqqSettings).toContain('resolveWebQQColorMode(webQQColorMode.value, koishiColorMode.value)')
    expect(webqqView).toContain('resolvedWebQQColorMode')
    expect(webqqView).toContain('`is-color-${resolvedWebQQColorMode}`')
  ...
  it('uses the configured WebQQ accent color for CSS variables', () => {
    ...
    expect(webqqThemeView).not.toContain("'--onebot-webqq-webqq-accent-surface': accentColor")
  ...
  it('closes WebQQ when clicking outside the capsule host', () => {
    ...
    expect(capsuleView).toContain("'.onebot-webqq-webqq__notice-menu'")
    expect(capsuleView).toContain("'.onebot-webqq-webqq__message-search-date-popover'")
    expect(capsuleView).toContain("'.onebot-webqq-webqq__scrollbar-overlay'")
    expect(capsuleView).not.toContain("'.onebot-webqq-webqq__secondary-page'")
  })
  ...
  it('renders contacts, groups, message history, and gated send input', () => {
    ...
    expect(webqqView).toContain('@keydown="handleComposerKeydown"')
    expect(webqqView).toContain('const composerDraft = ref<WebQQComposerDraft>(createEmptyWebQQComposerDraft())')
    expect(webqqView).toContain('...serializeWebQQComposerDraft(composerDraft.value.tokens)')
    expect(webqqView).toContain('WebQQMentionMenu')
    expect(webqqView).not.toContain('pendingMentionUserIds')
    expect(webqqView).not.toContain('class="onebot-webqq-webqq__send-mentions"')
    expect(webqqView).toContain('const contextHeight = context ? Math.ceil(context.getBoundingClientRect().height) + 8 : 0')
    expect(webqqView).toContain('webQQSendSpace.value = webQQSendHeight.value + contextHeight + 28')
    expect(webqqView).toContain("'--onebot-webqq-webqq-send-space': `${selectionMode.value ? 64 : webQQSendSpace.value}px`")
    expect(webqqView).toContain('ref="sendTextInput"')
    expect(webqqView).toContain('const requestInput = sendTextInput.value')
    expect(webqqView).toContain('await nextTick()')
    expect(webqqView).toContain('requestInput?.focus()')
    ...
    expect(webqqView).toContain("type WebQQSendFileKind = 'image' | 'video' | 'file'")
    expect(webqqView).toContain("if (mime.startsWith('video/')) return 'video'")
    expect(webqqView).toContain("return webQQVideoFileExtensions.has(extension) ? 'video' : 'file'")
    expect(webqqView).toContain('URL.createObjectURL(file)')
    expect(webqqView).toContain('ref="sendContext"')
    expect(webqqView).toContain('sendContextResizeObserver.observe(sendContext.value)')
    expect(webqqView).toContain('v-else-if="file.kind === \'video\'"')
    expect(webqqView).toContain('<video')
    expect(webqqView).toContain('preload="metadata"')
    const videoDraftSource = sourceBetween(webqqView, 'v-else-if="file.kind === \'video\'"', '<span v-else class="onebot-webqq-webqq__send-image">')
    expect(videoDraftSource).not.toContain('openLocalImagePreview')
    ...
    expect(webqqTypes).toContain("'image' | 'video' | 'file'")
    expect(serverWebqqTypesSource).toContain("'image' | 'video' | 'file'")
  ...
  it('cleans WebQQ live listeners and singleton client state on unmount or plugin dispose', () => {
    ...
    expect(entryState).not.toContain('availableBots.value = []')
    expect(entryState).not.toContain("selectedBotSelfId.value = ''")
    expect(onebotBots).toContain("const BOT_STATE_KEY = '__onebot_webqq_client_bots__'")
    expect(onebotBots).toContain('export const availableBots = botState.availableBots')
    expect(onebotBots).toContain('export const selectedBotSelfId = botState.selectedBotSelfId')
    expect(entryState).toContain("const ENTRY_STATE_KEY = '__onebot_webqq_client_entry__'")
    expect(entryState).toContain('export const webQQOpen = entryState.webQQOpen')
    expect(entryState).toContain('export const debug = entryState.debug')
  ...
  it('opens a WebQQ notification dropdown menu from the bell button', () => {
    ...
    expect(webqqSidebar).toContain('<Teleport to="body">')
    expect(webqqSidebar).toContain('<WebQQNoticeMenu')
    expect(webqqSidebar).toContain('onebot-webqq-webqq__notice-menu--desktop')
    expect(webqqSidebar).toContain('getDesktopNoticeMenuPosition')
  ...
  it('renders profile editing inside account fields instead of a separate form', () => {
    expect(webqqProfileCardView).toContain('class="onebot-webqq-webqq__profile-card-hero"')
    expect(webqqProfileCardView).toContain('class="onebot-webqq-webqq__profile-card-avatar-button"')
    ...
    expect(webqqProfileCardView).toContain('class="onebot-webqq-webqq__profile-card-field-action"')
    expect(webqqProfileCardView).toContain('class="onebot-webqq-webqq__profile-card-select"')
    expect(webqqProfileCardView).toContain('class="onebot-webqq-webqq__profile-card-select-trigger"')
    ...
    expect(webqqProfileCardView).not.toContain('onebot-webqq-webqq__profile-card-edit-actions')
  })
  ...
  it('searches the current WebQQ conversation history from the inline header search and jumps to a result', () => {
    ...
    expect(webqqView).toContain('messageSearchTrigger.value.focus({ preventScroll: true })')
    ...
    expect(webqqView).toContain('localDateToMessageSearchRange(messageSearchLocalDate.value)')
    expect(webqqView).toContain('searchWebQQMessages({')
    expect(webqqView).toContain('filterWebQQSearchMessages')
    expect(webqqView).toContain('loadCachedWebQQMessages(chat.type, chat.peerId)')
    expect(webqqView).toContain("reject(new Error('查找聊天记录超时'))")
    expect(webqqView).toContain('if (localMatches.length)')
    expect(webqqView).toContain('messageSearchNextBeforeSequence.value')
    expect(webqqView).toContain('messageList.value?.scrollToMessage(message.id || message.sequence)')
    expect(webqqMessageListView).toContain('defineExpose({')
    expect(webqqMessageListView).toContain('scrollToMessage: scrollToQuotedMessage')
    expect(webqqMessageSearchPage).toContain('class="onebot-webqq-webqq__message-search-field"')
    ...
    expect(webqqMessageSearchPage).toContain("searchInput.value?.focus({ preventScroll: true })")
    ...
    expect(webqqMessageSearchPage).toContain('popover="manual"')
    expect(webqqMessageSearchPage).toContain("enableWebQQFrostedGlass ? 'is-frosted' : 'is-plain'")
    expect(webqqMessageSearchPage).toContain('showWebQQPopover')
    expect(webqqMessageSearchPage).not.toContain('class="webqq-message-search')
    expect(webqqMessageSearchPage).toContain('class="onebot-webqq-webqq__message-search-calendar-grid"')
  ...
  it('attaches the WebQQ overlay scrollbar directive to primary and modal scroll areas', () => {
    ...
    expect(webqqMessageSearchPage).toContain('import { vWebqqScrollbar }')
    expect(webqqMessageSearchPage).toContain("v-webqq-scrollbar=\"{ tone: 'accent' }\"")
  ...
  it('renders WebQQ recall events and recalled message marks', () => {
    ...
    expect(webqqMessageReactionsView).toContain("import { getWebQQEmojiFace } from '../utils/emoji-catalog'")
    expect(webqqMessageReactionsView).toContain('getFace(reaction.emojiId)?.url')
    expect(webqqMessageReactionsView).toContain(':src="withProxy(getFace(reaction.emojiId)!.url)"')
    expect(webqqMessageReactionsView).toContain('function getReactionUserAvatar')
    expect(webqqMessageReactionsView).toContain(':src="withProxy(getReactionUserAvatar(user))"')
  ...
  it('renders QQ-style WebQQ reactions outside bubbles without user avatars', () => {
    ...
    expect(webqqMessageReactionsView).not.toContain('onebot-webqq-webqq__message-reaction-count')
    expect(webqqMessageReactionsView).not.toContain('shouldShowReactionCount')
  })
  ...
  it('renders group badges around sender names in opposite order by direction', () => {
    ...
    expect(webqqMessageListView).toContain('class="onebot-webqq-webqq__message-avatar-trigger"')
  ...
  it('shares the summed WebQQ unread count with the capsule state', () => {
    ...
    // 「配置镜像不再导出它」由 tests/webqq-members.test.ts 读运行时导出面断言，这里只钉 import 位置。
