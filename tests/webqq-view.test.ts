...
describe('webqq observer view', () => {
  ...
  it('renders contacts, groups, message history, and gated send input', () => {
    ...
    expect(webqqView).toContain("...pendingMentionUserIds.value.map((userId) => ({ type: 'at' as const, userId }))")
    expect(webqqView).toContain('pendingMentionUserIds.value = []')
    expect(webqqView).toContain('class="onebot-webqq-webqq__send-mentions"')
    expect(webqqView).not.toContain('const mention = `@${name} `')
    expect(webqqView).toContain('const replyHeight = replyingToMessage.value ? 46 : 0')
    expect(webqqView).toContain('webQQSendSpace.value = webQQSendHeight.value + attachmentHeight + replyHeight')
