...
const webqqMessageSearchModal = await readFile(new URL('../client/webqq/components/WebQQMessageSearchModal.vue', import.meta.url), 'utf8')
...
describe('webqq observer view', () => {
  ...
  it('searches the current WebQQ conversation history and jumps to a result', () => {
    ...
    expect(webqqView).toContain('<WebQQMessageSearchModal')
    ...
    expect(webqqMessageSearchModal).toContain('placeholder="输入关键词"')
    expect(webqqMessageSearchModal).toContain('继续搜索更早记录')
    expect(webqqMessageSearchModal).toContain('v-webqq-scrollbar class="onebot-webqq-webqq__message-search-results"')
  ...
  it('attaches the WebQQ overlay scrollbar directive to every WebQQ scroll area', () => {
    ...
    expect(webqqMessageSearchModal).toContain("import { vWebqqScrollbar } from '../utils/webqq-scrollbar'")
    expect(webqqMessageSearchModal).toContain('v-webqq-scrollbar class="onebot-webqq-webqq__message-search-results"')
