...
- 右下角聊天胶囊：显示当前 bot、最近会话、收发计数、ChatLuna 思考状态和 WebQQ 总未读数，可拖动到任意位置
...
按住胶囊任意位置拖动可以把它挪到别处，位置按浏览器记住，下次进控制台还在原处；拖动超过几个像素才算拖动，轻点仍然是打开观察窗或切换机器人。观察窗会跟着胶囊落位：默认贴着胶囊上沿打开，上方放不下时翻到胶囊下方，并始终夹在视口内。
